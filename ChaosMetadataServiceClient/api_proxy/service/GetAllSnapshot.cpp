...
std::unique_ptr<GetAllSnapshotHelper> GetAllSnapshot::getHelper(CDataWrapper *api_result) {
    return std::unique_ptr<GetAllSnapshotHelper>(new GetAllSnapshotHelper(api_result));
}
...
GetAllSnapshotHelper::GetAllSnapshotHelper(CDataWrapper *api_result) {
    ...
    std::unique_ptr<CMultiTypeDataArrayWrapper> snapshot_desc_list(api_result->getVectorValue("snapshot_list_result"));
    for(int idx = 0;
