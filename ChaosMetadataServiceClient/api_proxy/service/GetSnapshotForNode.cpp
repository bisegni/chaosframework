...
std::unique_ptr<GetSnapshotForNodeHelper> GetSnapshotForNode::getHelper(CDataWrapper *api_result) {
    return std::unique_ptr<GetSnapshotForNodeHelper>(new GetSnapshotForNodeHelper(api_result));
}
...
GetSnapshotForNodeHelper::GetSnapshotForNodeHelper(CDataWrapper *api_result) {
    ...
    std::unique_ptr<CMultiTypeDataArrayWrapper> snapshot_desc_list(api_result->getVectorValue("snapshot_for_node"));
    for(int idx = 0;
