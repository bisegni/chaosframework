...
std::unique_ptr<GetDomainsHelper> GetDomains::getHelper(CDataWrapper *api_result) {
    return std::unique_ptr<GetDomainsHelper>(new GetDomainsHelper(api_result));
}
...
GetDomainsHelper::GetDomainsHelper(CDataWrapper *api_result) {
    ...
    std::unique_ptr<CMultiTypeDataArrayWrapper> domains_vec(api_result->getVectorValue("domain_list"));
    for(int idx = 0;
