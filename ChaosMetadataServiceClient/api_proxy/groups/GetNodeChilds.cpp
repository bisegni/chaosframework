...
std::unique_ptr<GetNodeChildsHelper> GetNodeChilds::getHelper(CDataWrapper *api_result) {
    return std::unique_ptr<GetNodeChildsHelper>(new GetNodeChildsHelper(api_result));
}
...
GetNodeChildsHelper::GetNodeChildsHelper(CDataWrapper *api_result) {
    ...
    std::unique_ptr<CMultiTypeDataArrayWrapper> vec(api_result->getVectorValue("node_child_list"));
    for(int idx = 0;
        idx < vec->size();
        idx++) {
