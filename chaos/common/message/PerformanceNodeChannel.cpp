...
int PerformanceNodeChannel::getPerformanceSession(DirectIOPerformanceSession **performance_session_handler,
    ...
    CDWUniquePtr init_performance_session_param(new CDataWrapper());
    ...
    if(err == ErrorCode::EC_NO_ERROR) {
        CDWUniquePtr info_pack = init_session_result->getCSDataValue(RpcActionDefinitionKey::CS_CMDM_ACTION_MESSAGE);
