...
bool MultiAddressMessageChannel::serviceOnlineCheck(void *service_ptr) {
    ...
    int retry = 3;
    MMCFeederService *service = static_cast<MMCFeederService*>(service_ptr);
    std::unique_ptr<MessageRequestFuture> request = MessageChannel::echoTest(service->ip_port,
                                                                           NULL);
    while(--retry>0) {
        if(request->wait(500)) {
            retry = 0;
...
std::unique_ptr<MessageRequestFuture> MultiAddressMessageChannel::_sendRequestWithFuture(const std::string& action_domain,
                                                                                       const std::string& action_name,
                                                                                       CDataWrapper *request_pack,
                                                                                       std::string& used_remote_address) {
    std::unique_ptr<MessageRequestFuture> result;
    MMCFeederService *service =  static_cast<MMCFeederService*>(service_feeder.getService());
...
//!send an rpc request to a remote node
std::unique_ptr<MultiAddressMessageRequestFuture> MultiAddressMessageChannel::sendRequestWithFuture(const std::string& action_domain,
                                                                                                  const std::string& action_name,
                                                                                                  CDataWrapper *request_pack,
                                                                                                  int32_t request_timeout) {
    return std::unique_ptr<MultiAddressMessageRequestFuture>(new MultiAddressMessageRequestFuture(this,
                                                                                                action_domain,
                                                                                                action_name,
                                                                                                request_pack,
                                                                                                request_timeout));
