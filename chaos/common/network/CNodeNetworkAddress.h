	...
	namespace common {
		namespace network {
			...
			struct CNetworkAddress {
				...
				std::string ip_port;
                CNetworkAddress(){};
                CNetworkAddress(const CNetworkAddress& cna):ip_port(cna.ip_port){};
                CNetworkAddress(const std::string& _ip_port):ip_port(_ip_port){};
			};
			...
			struct CNodeNetworkAddress : public CNetworkAddress {
				...
				std::string node_id;
                CNodeNetworkAddress(){};
                CNodeNetworkAddress(const CNodeNetworkAddress& cnna):CNetworkAddress(cnna.ip_port),node_id(cnna.node_id){};
                CNodeNetworkAddress(const std::string& _node_id,
                                    const std::string& _ip_port):CNetworkAddress(_ip_port),node_id(_node_id){};
			};
			...
			struct CDeviceNetworkAddress : public CNodeNetworkAddress{
                ...
                CDeviceNetworkAddress(const CDeviceNetworkAddress& cdna):CNodeNetworkAddress(cdna.node_id,
                                                                                             cdna.ip_port),device_id(cdna.device_id){};
                CDeviceNetworkAddress(const std::string _device_id,
                                      const std::string& _node_id,
                                      const std::string& _ip_port):CNodeNetworkAddress(_node_id,
                                                                                      _ip_port),device_id(_device_id){};
			};
		}
	}
}
