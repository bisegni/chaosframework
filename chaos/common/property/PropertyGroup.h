            ...
            void deserialize(chaos::common::data::CDataWrapper *serialized_data) {
                ...
                if(serialized_data->hasKey("property_g_plist") &&
                   serialized_data->isVectorValue("property_g_plist")){
                    ...
                    std::unique_ptr<chaos::common::data::CMultiTypeDataArrayWrapper> prop_array(serialized_data->getVectorValue("property_g_plist"));
                    
                    for(unsigned int idx = 0;
                        idx < prop_array->size();
                        idx++) {
                        std::unique_ptr<chaos::common::data::CDataWrapper> prop(prop_array->getCDataWrapperElementAtIndex(idx));
