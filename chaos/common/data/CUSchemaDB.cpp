...
void CUSchemaDB::addDeviceId(const string& deviceID) {
    ...
    if(dsEntity) {
        ...
        addUniqueAttributeProperty(dsEntity, mapDatasetKeyForID[ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_TIMESTAMP], timingUtils.getTimeStamp(), false);
    }
}
...
void CUSchemaDB::addAttributeToDataSet(const char*const attributeDeviceID,
    ...
    if(elementDst.get()) {
        ...
        addUniqueAttributeProperty(elementDst.get(), mapDatasetKeyForID[ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_VALUE_MAX_SIZE], (int64_t)typeMaxDimension);
...
void CUSchemaDB::addAttributeToDataSetFromDataWrapper(CDataWrapper& attributeDataWrapper) {
    ...
    if(attributeDataWrapper.hasKey(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_DESCRIPTION)){
        ...
        elementsDescriptions.reset(attributeDataWrapper.getVectorValue(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_DESCRIPTION));
