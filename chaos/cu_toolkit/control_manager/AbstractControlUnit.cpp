    ...
    void         AbstractControlUnit::doInitRpCheckList() {
        ...
        CHAOS_CHECK_LIST_START_SCAN_TO_DO(check_list_sub_service, "_init") {
            ...
            CHAOS_CHECK_LIST_DONE(check_list_sub_service, "_init", INIT_RPC_PHASE_UPDATE_CONFIGURATION) {
                ...
                CDWUniquePtr dataset_value(new CDataWrapper());
                //launch the handler
                init_configuration->copyKeyTo(NodeDefinitionKey::NODE_UNIQUE_ID, *dataset_value);
                if(init_configuration->hasKey(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_DESCRIPTION)){
                    ...
                    if(dataset_object->hasKey(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_DESCRIPTION)) {
                        ...
                        CMultiTypeDataArrayWrapperSPtr descriptions = dataset_object->getVectorValue(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_DESCRIPTION);
                        for (int idx = 0; idx < descriptions->size(); idx++) {
                            //next element in dataset
                            CDWUniquePtr element = descriptions->getCDataWrapperElementAtIndex(idx);
                            //attribute name
                        
                            if(!element->hasKey(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_ATTRIBUTE_NAME))
                                continue;
                            if(!element->hasKey(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_DEFAULT_VALUE))
                                continue;
                            element->copyKeyToNewKey(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_DEFAULT_VALUE,
                                                     element->getStringValue(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_ATTRIBUTE_NAME),
                                                     *dataset_value);
//                            dataset_value->addVariantValue(element->getStringValue(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_ATTRIBUTE_NAME),
//                                                           element->getVariantValue(ControlUnitNodeDefinitionKey::CONTROL_UNIT_DATASET_DEFAULT_VALUE));
                        }
                    }
                }
                setDatasetAttribute(MOVE(dataset_value));
...
if (attributeInfo.maxRange.size() && v > attributeInfo.maxRange) throw MetadataLoggingCException(getCUID(), -1, boost::str(boost::format("Invalid value (%1%) [max:%2% Min:%3%] for attribute %4%") % v % attr_name % attributeInfo.minRange % attributeInfo.maxRange).c_str(), __PRETTY_FUNCTION__);
    ...
    CDWUniquePtr AbstractControlUnit::setDatasetAttribute(CDWUniquePtr dataset_attribute_values) {
        ...
        w_lock->lock();
        ACULAPP_ << dataset_attribute_values->getJSONString();
        RangeValueInfo           attributeInfo;
                ...
                for (std::vector<std::string>::iterator iter = in_attribute_name.begin();
                    ...
                    if (dataset_attribute_values->hasKey(attr_name)) {
                        ...
                        switch (attribute_cache_value->type) {
                            case DataType::TYPE_BOOLEAN: {
                                bool bv = dataset_attribute_values->getVariantValue(attr_name).asBool();
                                attribute_cache_value->setValue(&bv, sizeof(bool));
                                break;
                            }
                            case DataType::TYPE_INT32: {
                                int32_t i32v = dataset_attribute_values->getVariantValue(attr_name).asInt32();
                                CHECK_FOR_RANGE_VALUE(int32_t, i32v, attr_name)
                                attribute_cache_value->setValue(&i32v, sizeof(int32_t));
                                break;
                            }
                            case DataType::TYPE_INT64: {
                                int64_t i64v = dataset_attribute_values->getVariantValue(attr_name).asInt64();
                                CHECK_FOR_RANGE_VALUE(int64_t, i64v, attr_name)
                                attribute_cache_value->setValue(&i64v, sizeof(int64_t));
                                break;
                            }
                            case DataType::TYPE_DOUBLE: {
                                double dv = dataset_attribute_values->getVariantValue(attr_name).asDouble();
