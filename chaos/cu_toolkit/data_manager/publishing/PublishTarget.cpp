...
bool PublishTarget::publish(const PublishableElement& publishable_element) {
    ...
    std::unique_ptr<SerializationBuffer> serialization(getDataPack(publishable_element.dataset_ptr->dataset_value_cache)->getBSONData());
...
std::unique_ptr<CDataWrapper> PublishTarget::getDataPack(const chaos::common::data::cache::AttributeCache &attribute_cache) {
    //clock the cache
...
void *PublishTarget::serviceForURL(const URL &url, uint32_t service_index) {
    ...
    std::unique_ptr<PublishTargetClientChannel> endpoint_connection(new PublishTargetClientChannel());
