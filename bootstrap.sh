...
#*	Created by Claudio Bisegni & Andrea MIchelotti
#*
...
if [ "$CHAOS_TARGET" == "armhf" ]; then
    ...
    if arm-linux-gnueabihf-gcc-4.8 -v 2>&1 | grep version; then
	...
	echo "## no arm-linux-gnueabihf-gcc-4.8 in PATH"
	exit 1
    fi
else
	...
	if [ "$CHAOS_TARGET" == "i686-linux26" ] ; then
	    ...
	    if i686-nptl-linux-gnu-gcc -v 2>&1 | grep version; then
		...
		echo "## NO i686-nptl-linux-gnu-gcc in PATH"
		exit 1
		fi
