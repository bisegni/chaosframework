...
public class DatasetAttribute extends BSONBusinessObject {
	...
	public void fillFromBson(Object bson) throws RefException {
		...
		if (bsonAttributeDescription.containsField(RPCConstants.DATASET_ATTRIBUTE_MAX_RANGE)) {
			this.rangeMax = (checkEmptyString(bsonAttributeDescription.getString(RPCConstants.DATASET_ATTRIBUTE_MAX_RANGE)));
		}
		if (bsonAttributeDescription.containsField(RPCConstants.DATASET_ATTRIBUTE_MIN_RANGE)) {
			this.rangeMin =(checkEmptyString(bsonAttributeDescription.getString(RPCConstants.DATASET_ATTRIBUTE_MIN_RANGE)));
		}
		if (bsonAttributeDescription.containsField(RPCConstants.DATASET_ATTRIBUTE_DEFAULT_VALUE)) {
			this.defaultValue =(checkEmptyString(bsonAttributeDescription.getString(RPCConstants.DATASET_ATTRIBUTE_DEFAULT_VALUE)));
		}
	}
	...
	public void checkDefaultValueForType(String strValueToCheck) throws RefException {
		try {
			...
			throw new RefException("Attribute:" +name +": the value \"" + strValueToCheck + " can't be associated to the type");
		}
	...
	if (defaultValue != null) {
	    switch (getType()) {
				...
				case 2:{
					...
					double minv = getRangeMin()!=null? Double.parseDouble(getRangeMin()):-Double.MAX_VALUE;
					defMinMinValue = (defv<minv);
					defMaxMaxValue = (defv>maxv);
					//					System.out.println("min: "+minv+" max:"+maxv+" default:"+defv + " def min value:"+defMinMinValue);
			...
			if (defMinMinValue) {
			    throw new RefException(String.format("The default value %s (type %d) is lower than the minimum value \"%s\"",defaultValue, getType(), getRangeMin()));
			}
			if (defMaxMaxValue) {
			    throw new RefException(String.format("The default value %s (type %d) is greater than the maximum value \"%s\"",defaultValue, getType(), getRangeMax()));
			}
