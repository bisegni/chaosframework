...
bool
NodeConnectionInteraction::
canConnect(PortIndex &portIndex, bool& typeConversionNeeded, std::unique_ptr<NodeModel> & converterModel) const
{
  ...
  auto const   &modelTarget = _node->nodeModel();
  NodePortType candidateNodePortType = modelTarget->dataType(requiredPort, portIndex);

  if (connectionDataType.id != candidateNodePortType.id)
  {
    if (requiredPort == PortType::In)
    {
      return typeConversionNeeded = (converterModel = _scene->registry().getTypeConverter(connectionDataType.id, candidateNodePortType.id)) != nullptr;
    }
    return typeConversionNeeded = (converterModel = _scene->registry().getTypeConverter(candidateNodePortType.id, connectionDataType.id)) != nullptr;
  }
...
bool NodeConnectionInteraction::tryConnect() const
{
  ...
  std::unique_ptr<NodeModel> typeConverterModel;
  ...
  if (typeConversionNeeded)
  {
    ...
    //Connecting the converter node to the two nodes that originally supposed to be connected.
...
disconnect(PortType portToDisconnect) const
{
  ...
  QtNodes::NodeState &state = _node->nodeState();
