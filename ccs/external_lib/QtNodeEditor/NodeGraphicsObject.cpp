...
embedQWidget()
{
    ...
    if (auto w = _node.nodeModel()->embeddedWidget())
    {
...
mousePressEvent(QGraphicsSceneMouseEvent * event)
{
    ...
    QtNodes::NodeGeometry & nodeGeometry = _node.nodeGeometry();

    // TODO do not pass sceneTransform
    int portIndex = nodeGeometry.checkHitScenePoint(PortType::Out,
                                                    event->scenePos(),
                                                    sceneTransform());

    if (portIndex != QtNodes::INVALID)
    {
        QtNodes::NodeState& nodeState = _node.nodeState();

        std::unordered_map<QUuid, QtNodes::Connection*>& connections =
                nodeState.connections(PortType::Out, portIndex);

        int max_connections = _node.nodeModel()->maxConnectionsPerPort(PortType::Out, portIndex);

        bool initialize_new_connection = (max_connections == -1);

        if ( max_connections == 1)
        {
            if( connections.size() >= 1 )
            {
                QtNodes::Connection* con = connections.begin()->second;
                _scene.deleteConnection( *con );
                initialize_new_connection = true;
            }
            initialize_new_connection = true;
        }
        else{
            if( max_connections > 1 && connections.size() < max_connections){
                initialize_new_connection = true;
            }
        }


        if(initialize_new_connection)
        {
            // todo add to FlowScene
            auto connection = _scene.createConnection(PortType::Out,
                                                      _node,  portIndex);

            _node.nodeState().setConnection(PortType::Out,
                                            portIndex, *connection);
