...
int MongoDBDriver::setNodeHealthData(const std::string& node_uid,
                                     chaos::common::data::CDataWrapper& health_data) {
    int err = 0;
    
    try {
        common::data::structured::HealthStatSDWrapper attribute_reference_wrapper;
        
        attribute_reference_wrapper.deserialize(&health_data);
        
        
        mongo::BSONObj q = BSON(chaos::NodeDefinitionKey::NODE_UNIQUE_ID << attribute_reference_wrapper().node_uid);
        mongo::BSONObj u = BSON("$set" << BSON("health_stat" << BSON(NodeHealtDefinitionKey::NODE_HEALT_TIMESTAMP << mongo::Date_t(attribute_reference_wrapper().timestamp) <<
                                                                     NodeHealtDefinitionKey::NODE_HEALT_PROCESS_UPTIME << (long long)attribute_reference_wrapper().uptime <<
                                                                     NodeHealtDefinitionKey::NODE_HEALT_USER_TIME << attribute_reference_wrapper().user_time <<
                                                                     NodeHealtDefinitionKey::NODE_HEALT_SYSTEM_TIME << attribute_reference_wrapper().sys_time <<
                                                                     NodeHealtDefinitionKey::NODE_HEALT_STATUS << attribute_reference_wrapper().health_status)));
        DEBUG_CODE(MDBID_LDBG_ << "setNodeHealthData count ---------------------------------------------";)
        DEBUG_CODE(MDBID_LDBG_ << "condition" << q;)
        DEBUG_CODE(MDBID_LDBG_ << "update" << u;)
        DEBUG_CODE(MDBID_LDBG_ << "setNodeHealthData count ---------------------------------------------";)
        
        if((err = ha_connection_pool->update(MONGO_DB_COLLECTION_NAME_COL(db_name, "mds_nodes"),
                                             q,
                                             u,
                                             false,
                                             false,
                                             mongo::WriteConcern::acknowledged))){
            MDBID_LERR_ << CHAOS_FORMAT("Error updating health stat information for node %1%",%node_uid);
        }
    } catch (const mongo::DBException &e) {
        MDBID_LERR_ << e.what();
        err = e.getCode();
