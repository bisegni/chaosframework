...
int VFSFile::giveHeartbeat(uint64_t hb_time) {
    ...
    if((err = db_driver_ptr->vfsSetHeartbeatOnDatablock(this, current_data_block, hb_time))) {
...
int VFSFile::openKeyToJournal(std::string journal_key) {
    int err = 0;
    if(!current_journal_data_block) return -1;
...
int VFSFile::ensureDatablock(bool force_switch) {
    return 0;
}
...
int VFSFile::mantain(int closing_state) {
    ...
    if(!isDataBlockValid(current_data_block)) {
        if((current_data_block != NULL)&&
           (err = closeCurrentDataBlock(closing_state))) {
...
int VFSFile::syncWorkPositionFromDatablockAndJournal(DataBlock *datablock, DataBlock *current_journal_data_block) {
    int err = 0;
    if(!datablock) return -1;
    if(!current_journal_data_block) return -2;
