...
CacheDriver* CacheDriverPool::allocateResource(const std::string& pool_identification,
    ...
    try{
        ...
        InizializableService::initImplementation(pooled_driver,
                                                 &ChaosDataService::getInstance()->setting.cache_driver_setting,
...
void CacheDriverPool::init(void *init_data) throw (chaos::CException) {
    ...
    if(!ChaosDataService::getInstance()->setting.cache_driver_setting.cache_driver_impl.size()) {LOG_AND_TROW(DP_LOG_ERR, -1, "No cache implemetation provided");}
    if(!ChaosDataService::getInstance()->setting.cache_driver_setting.startup_chache_servers.size()) {LOG_AND_TROW(DP_LOG_ERR, -2, "No cache servers provided")}
}
