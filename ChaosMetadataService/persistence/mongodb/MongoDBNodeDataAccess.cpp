...
int MongoDBNodeDataAccess::insertNewNode(CDataWrapper& node_description) {
    ...
    try {
        ...
        std::unique_ptr<SerializationBuffer> ser(node_description.getBSONData());
...
int MongoDBNodeDataAccess::updateNode(chaos::common::data::CDataWrapper& node_description) {
    ...
    try {
        ...
        if(node_description.hasKey(chaos::RpcActionDefinitionKey::CS_CMDM_ACTION_DESC)) {
            std::unique_ptr<CMultiTypeDataArrayWrapper> action_array(node_description.getVectorValue(chaos::RpcActionDefinitionKey::CS_CMDM_ACTION_DESC));
            for(int idx = 0;
                idx < action_array->size();
                idx++) {
                mongo::BSONObjBuilder action_description;
                mongo::BSONArrayBuilder param_descriptions;
                std::unique_ptr<CDataWrapper> element(action_array->getCDataWrapperElementAtIndex(idx));
                ...
                if(element->hasKey(chaos::RpcActionDefinitionKey::CS_CMDM_ACTION_DESC_PARAM)) {
                    std::unique_ptr<CMultiTypeDataArrayWrapper> param_array(element->getVectorValue(chaos::RpcActionDefinitionKey::CS_CMDM_ACTION_DESC_PARAM));
                    for(int idx = 0;
                        idx < param_array->size();
                        idx++) {
                        mongo::BSONObjBuilder single_param_desc;
                        std::unique_ptr<CDataWrapper> param(param_array->getCDataWrapperElementAtIndex(idx));
...
int MongoDBNodeDataAccess::setCommand(chaos::common::data::CDataWrapper& command) {
    ...
    try {
        ...
        std::unique_ptr<SerializationBuffer> ser(command.getBSONData());
...
int MongoDBNodeDataAccess::setCommandTemplate(chaos::common::data::CDataWrapper& command_template) {
    ...
    try {
        ...
        std::unique_ptr<CDataWrapper> to_update(new CDataWrapper());
        ...
        std::unique_ptr<SerializationBuffer> chaos_bson(to_update->getBSONData());
