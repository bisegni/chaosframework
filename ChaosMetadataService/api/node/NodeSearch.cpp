...
chaos::common::data::CDataWrapper *NodeSearch::execute(chaos::common::data::CDataWrapper *api_data,
  ...
  if (!api_data){
	LOG_AND_TROW(NS_ERR,
				 -1,
				 "Search parameter are needed");
  }

  if (!api_data->hasKey("unique_id_filter")){
	LOG_AND_TROW(NS_ERR,
				 -2,
				 "Filter for node unique id is madnatory");
  }

  if (!api_data->hasKey("node_type_filter")){
	LOG_AND_TROW(NS_ERR,
				 -3,
				 "Filter for node type is madnatory");
  }
