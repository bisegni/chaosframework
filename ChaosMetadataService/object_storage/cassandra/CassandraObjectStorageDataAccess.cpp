...
cass_sess_shrd_ptr(_cass_sess_shrd_ptr){
    ...
    /* DELETE: */
    MAKE_MANAGED_FUTURE(prepare_future_2, cass_session_prepare(cass_sess_shrd_ptr.get(), CHAOS_FORMAT("DELETE FROM %1% WHERE %2% = ? AND %3% = ?",
                                                                                                      %DAQ_TABLE_NAME
                                                                                                      %chaos::DataPackCommonKey::DPCK_DEVICE_ID
                                                                                                      %chaos::DataPackCommonKey::DPCK_TIMESTAMP).c_str()));
    ...
    /* GET LAST: */
    MAKE_MANAGED_FUTURE(prepare_future_3,cass_session_prepare(cass_sess_shrd_ptr.get(), CHAOS_FORMAT("SELECT * FROM %1% WHERE %2% = ? AND %3% = ? ORDER BY %4% DESC LIMIT 1",
                                                                                                     %DAQ_TABLE_NAME
                                                                                                     %chaos::DataPackCommonKey::DPCK_DEVICE_ID
                                                                                                     %chaos::DataPackCommonKey::DPCK_TIMESTAMP
                                                                                                     %chaos::DataPackCommonKey::DPCK_TIMESTAMP).c_str()));
    CHECK_FUTURE(get_last_daq_prepared, prepare_future_3, -4);


    /* FIND the first page_len records between timestamp_from and timestamp_to */
    //NORMAL
    MAKE_MANAGED_FUTURE(prepare_future_4, cass_session_prepare(cass_sess_shrd_ptr.get(), CHAOS_FORMAT("SELECT * FROM %1% WHERE %2% = ? AND %3% >= ? AND %4% <= ? AND %5% >= ? AND %6% >= ?",
                                                                                                      %DAQ_TABLE_NAME
                                                                                                      %chaos::DataPackCommonKey::DPCK_DEVICE_ID
                                                                                                      %chaos::DataPackCommonKey::DPCK_TIMESTAMP
                                                                                                      %chaos::DataPackCommonKey::DPCK_TIMESTAMP
    ...
    CHECK_FUTURE(find_daq_prepared, prepare_future_4, -5);

    //REVERSE
    MAKE_MANAGED_FUTURE(prepare_future_5, cass_session_prepare(cass_sess_shrd_ptr.get(), CHAOS_FORMAT("SELECT * FROM %1% WHERE %2% >= ? and %3% <= ? AND %4% <= ? AND %5% <= ?",
                                                                                                      %DAQ_TABLE_NAME
                                                                                                      %chaos::DataPackCommonKey::DPCK_TIMESTAMP
                                                                                                      %chaos::DataPackCommonKey::DPCK_TIMESTAMP
                                                                                                      %chaos::ControlUnitDatapackCommonKey::RUN_ID
                                                                                                      %chaos::DataPackCommonKey::DPCK_SEQ_ID).c_str()));
    CHECK_FUTURE(find_daq_prepared_reverse, prepare_future_5, -6);
    
    /* COUNT: */
    MAKE_MANAGED_FUTURE(prepare_future_6, cass_session_prepare(cass_sess_shrd_ptr.get(), CHAOS_FORMAT("SELECT COUNT(*) FROM %1%", %DAQ_TABLE_NAME).c_str()));
    CHECK_FUTURE(count_daq_prepared, prepare_future_6, -7);
}
...
int CassandraObjectStorageDataAccess::pushObject(const std::string&                       key,
    ...
    cass_statement_bind_int64_by_name(statement.get(),
                                      ...
                                      stored_object.getInt64Value(chaos::ControlUnitNodeDefinitionKey::CONTROL_UNIT_RUN_ID));
    cass_statement_bind_int64_by_name(statement.get(),
                                      chaos::DataPackCommonKey::DPCK_SEQ_ID,
    ...
    MAKE_MANAGED_FUTURE(query_future, cass_session_execute(cass_sess_shrd_ptr.get(), statement.get()));
