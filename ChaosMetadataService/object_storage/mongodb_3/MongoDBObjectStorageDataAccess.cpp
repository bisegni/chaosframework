...
int MongoDBObjectStorageDataAccess::pushObject(const std::string& key,
    ...
    try {
        ...
        bsoncxx::document::value buffer_stream = bsoncxx::builder::stream::document{} << MONGODB_DAQ_DATA_FIELD << buffer << bsoncxx::builder::stream::finalize;

        builder.append(bsoncxx::builder::concatenate(buffer_stream.view()));
