            ...
            case ErrorCode::EC_MDS_NODE_REGISTRATION_OK:
                ...
                if(ack_pack->hasKey("agent_description") &&
                   ack_pack->isCDataWrapperValue("agent_description")) {
                    std::unique_ptr<CDataWrapper> aget_desc_ser(ack_pack->getCSDataValue("agent_description"));
...
std::unique_ptr<CDataWrapper> AgentRegister::getAgentRegistrationPack() {
    ...
    for(MapWorkerIterator iter = map_worker.begin();
        iter != map_worker.end();
        iter++) {
        std::unique_ptr<CDataWrapper> worker_definition(new CDataWrapper());
    ...
    switch(registration_state()) {
        ...
        case AgentRegisterStateRegistering: {
            ...
            if(((reg_retry_counter++)%max_reg_retry_counter) == 0) {
                std::unique_ptr<CDataWrapper> reg = getAgentRegistrationPack();
