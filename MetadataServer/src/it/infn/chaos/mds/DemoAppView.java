...
public class DemoAppView extends RefVaadinBasePanel implements ItemClickListener {
	...
	public static final String	DATASET_SELECTED			= "DATASET_SELECTED";
	public static final String	ATTRIBUTE_EDITING			= "ATTRIBUTE_EDITING";
	public static final String	ATTRIBUTE_SAVE				= "ATTRIBUTE_SAVE";
	public static final String	DELETE_SELECTED_DEVICE		= "DELETE_SELECTED_DEVICE";
	public static final String	UPDATE_DEVICE_LIST			= "UPDATE_DEVICE_LIST";
	public static final String	UPDATE_DS_ATTRIBUTE_LIST	= "UPDATE_DS_ATTRIBUTE_LIST";
	...
	public static final String	TAB2_ATTR_NUMBER			= "Num. Of Attribute";
	...
	public void initGui() {
		...
		mv.getButtonRefresh().addListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				notifyEventoToControllerWithData(UPDATE_DEVICE_LIST, event.getSource(), null);
			}
		});
		...
		mv.getButtonDelete().addListener(new ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				notifyEventoToControllerWithData(DELETE_SELECTED_DEVICE, event.getSource(), null);
			}
		});
		...
		mv.getButtonRefresh().addListener(new ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				notifyEventoToControllerWithData(UPDATE_DS_ATTRIBUTE_LIST, event.getSource(), null);
			}
		});
		...
		mv.getTableDatasetAttribute().addActionHandler(new Action.Handler() {
			...
			public void handleAction(Action action, Object sender, Object target) {
				...
				if (ACTION_EDIT == action) {
					...
					mv.getTableDatasetAttribute().setEditable(!mv.getTableDatasetAttribute().isEditable());
					mv.getTableDatasetAttribute().refreshRowCache();
					notifyEventoToControllerWithData(ATTRIBUTE_SAVE, null, null);
	...
	public void update(Observable source, Object sourceData) {
		if (sourceData instanceof ViewNotifyEvent) {
			...
			if (viewEvent.getEventKind().equals(DemoAppView.ATTRIBUTE_EDITING)) {
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (event.getSource().equals(mv.getTableDevice())) {
			notifyEventoToControllerWithData(DEVICE_SELECTED, event.getSource(), event.getItemId());
		} else if (event.getSource().equals(mv.getTableDataset())) {
			notifyEventoToControllerWithData(DATASET_SELECTED, event.getSource(), event.getItemId());
		} else if (event.getSource().equals(mv.getTableDatasetAttribute())) {
			if (event.isDoubleClick()) {
				notifyEventoToControllerWithData(ATTRIBUTE_EDITING, event.getSource(), event.getItemId());
			}
		}
	}
