...
int MongoDBHAConnectionManager::runCommand(mongo::BSONObj& result,
    ...
    GET_CONNECTION()
    try {
        EXECUTE_MONGOAPI(if(!conn->get()->runCommand(ns, command, result, queryOptions)) {
...
int MongoDBHAConnectionManager::findAndModify(mongo::BSONObj& result,
    ...
    GET_CONNECTION()
    try {
        EXECUTE_MONGOAPI(result = conn->get()->findAndModify(ns,
                                                             query,
                                                             update,
                                                             upsert,
                                                             returnNew,
...
int MongoDBHAConnectionManager::findAndRemove(mongo::BSONObj& result,
    ...
    GET_CONNECTION()
    try {
        EXECUTE_MONGOAPI(result = conn->get()->findAndRemove(ns,
                                                             query,
                                                             sort,
                                                             fields););
    } catch (std::exception& ex) {
...
mongo::BSONObj MongoDBHAConnectionManager::distinct(const std::string &ns,
    ...
    GET_CONNECTION()
    try {
        EXECUTE_MONGOAPI(result = conn->get()->distinct(ns,
                                                        field,
                                                        query););
    } catch (std::exception& ex) {
        MDBHAC_LERR_ << "MongoDBHAConnectionManager::findOne" << " -> " << ex.what();
